...
def dump(context, tspec, fieldname, data, obj):
    if obj is None:
        return ""
    ...
    fname = "_".join(str(data[key]) for key in tspec.primary_keys)
...
def insert(context, data, tspec: TableSpec):
    ...
    for fname, ftype in tspec.fields.__dict__.items():
        if context.use_external_storage and ftype.lower() in ['blob', 'ndarray']:
            fpath = dump(context, tspec, fname, data, data[fname])
            _data = _data + (fpath, )
        else:
            _data = _data + (data[fname], )
...
def setup_db(db_path: Path, table_storage_root: Path | None):
